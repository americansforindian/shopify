...
if( !function_exists('trendz_archive_blog_post_defaults') ) {
    function trendz_archive_blog_post_defaults() {
        $defaults = array(
            'post-layout'      => 'entry-cover',
...
if( !function_exists('trendz_archive_blog_post_misc_defaults') ) {
    function trendz_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('trendz_archive_blog_post_params_default') ) {
    function trendz_archive_blog_post_params_default() {
        $params = array(
            ...
            'archive_meta_elements'  => array( 'author', 'date' ),
            'archive_readmore_text'  => esc_html__('Read More', 'trendz'),
...
if( !function_exists('trendz_archive_blog_post_defaults_filter') ) {
    function trendz_archive_blog_post_defaults_filter() {
        $defaults = trendz_archive_blog_post_defaults();
        return apply_filters( 'trendz_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('trendz_archive_blog_post_misc_defaults_filter') ) {
    function trendz_archive_blog_post_misc_defaults_filter() {
        $defaults = trendz_archive_blog_post_misc_defaults();
        return apply_filters( 'trendz_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('trendz_archive_blog_post_params') ) {
    function trendz_archive_blog_post_params() {
        $params = trendz_archive_blog_post_params_default();
        return apply_filters( 'trendz_archive_blog_post_params', $params );
    }
}

if( !function_exists('trendz_get_archive_post_combine_class') ) {
	function trendz_get_archive_post_combine_class() {

        $blog_defaults = trendz_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'trendz_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('trendz_get_archive_post_holder_class') ) {
	function trendz_get_archive_post_holder_class() {

        $blog_defaults = trendz_archive_blog_post_defaults_filter();
        $blog_misc_defaults = trendz_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'trendz_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('trendz_get_archive_post_style') ) {
	function trendz_get_archive_post_style() {
		...
		return apply_filters( 'trendz_get_archive_post_style', $post_style );
	}
}

if( !function_exists('trendz_get_archive_post_column') ) {
	function trendz_get_archive_post_column() {
		...
		return apply_filters( 'trendz_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'trendz_archive_set_post_class', 10, 3);
if( !function_exists('trendz_archive_set_post_class') ) {
    function trendz_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = trendz_archive_blog_post_params();
...
add_action( 'trendz_after_main_css', 'trendz_blog_enqueue_css', 10 );
if( !function_exists( 'trendz_blog_enqueue_css' ) ) {
	function trendz_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, TRENDZ_THEME_VERSION, 'all');
        ...
        if ( file_exists( get_theme_file_path('/modules/blog/templates/'.$post_style.'/assets/css/blog-archive-'.$post_style.'.css') ) ) {
            wp_enqueue_style( 'wdt-blog-archive-'.$post_style, get_theme_file_uri('/modules/blog/templates/'.$post_style.'/assets/css/blog-archive-'.$post_style.'.css'), false, TRENDZ_THEME_VERSION, 'all');
        }

		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, TRENDZ_THEME_VERSION, 'all' );
	}
}

add_action( 'trendz_before_enqueue_js', 'trendz_blog_enqueue_js' );
if( !function_exists( 'trendz_blog_enqueue_js' ) ) {
	function trendz_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = trendz_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'trendz_blog_archive_pagination', trendz_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'trendz_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'trendz_excerpt' ) ) {
	function trendz_excerpt( $limit = NULL ) {
